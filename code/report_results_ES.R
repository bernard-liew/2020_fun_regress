...
fileNames <- list.files("output/", "kerasScra_ES50")
...
fileNames <- list.files("output/", "kerasTL_ES30")
...
p <- ggplot (df.plot) +
  ...
  scale_color_manual(values = c("black", "red", "blue", "green"),
                     label = c("Observed", bquote(ML[fregress]), bquote(ML[DNN]), bquote(ML[TL]))) +
...
tiff("manuscript/temp_fig1_ES.tiff", units="in", width=9, height=5, res=300)
p
dev.off()
...
my_path <- paste0("manuscript/temp_table2_ES",
                  "perf",
                  ".docx")
